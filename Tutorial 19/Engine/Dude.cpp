...
{
	if (ms.LeftIsPressed()) {
		auto mousePos = ms.GetPos();
		dest = { (float)mousePos.first, (float)mousePos.second };		
	}

	Vec2 center = pos + Vec2{ width / 2, height / 2 };
	Vec2 normDif = (dest - center).GetNormalized();
	pos += normDif * speed * dt;
}
